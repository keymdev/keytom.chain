...
const (
	// MainnetChainID defines the Keytom EIP155 chain ID for mainnet
	MainnetChainID = "keym_2312"
	// TestnetChainID defines the Keytom EIP155 chain ID for testnet
	TestnetChainID = "keym_2313"
)

// IsMainnet returns true if the chain-id has the Keytom mainnet EIP155 chain prefix.
func IsMainnet(chainID string) bool {
	return strings.HasPrefix(chainID, MainnetChainID)
}

// IsTestnet returns true if the chain-id has the Keytom testnet EIP155 chain prefix.
...
// GetKeytomAddressFromBech32 returns the sdk.Account address of given address,
// while also changing bech32 human readable prefix (HRP) to the value set on
// the global sdk.Config (eg: `keytom`).
// The function fails if the provided bech32 address is invalid.
func GetKeytomAddressFromBech32(address string) (sdk.AccAddress, error) {
