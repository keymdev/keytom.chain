...
import (
	...
	"github.com/keytom/keytom.chain/v1/testutil/network"
)
...
Example:
	keytomd testnet init-files --v 4 --output-dir ./.testnets --starting-ip-address 192.168.10.2
	`,
	...
	cmd.Flags().String(flagNodeDaemonHome, "keytomd", "Home directory of the node's daemon configuration")
...
Example:
	keytomd testnet --v 4 --output-dir ./.testnets
	`,
		RunE: func(cmd *cobra.Command, _ []string) error {
			args := startArgs{}
			args.outputDir, _ = cmd.Flags().GetString(flagOutputDir)
...
func initTestnetFiles(
	...
	if args.chainID == "" {
		args.chainID = fmt.Sprintf("keym%d-1", tmrand.Int63n(9999999999999)+1)
	}
	...
	for i := 0; i < args.numValidators; i++ {
		...
		kb, err := keyring.New(sdk.KeyringServiceName(), args.keyringBackend, nodeDir, inBuf, keytomhd.Option())
		if err != nil {
			return err
		}
