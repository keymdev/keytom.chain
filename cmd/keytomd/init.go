...
import (
	...
	keytomcfg "github.com/keytom/keytom.chain/v1/cmd/config"
)
...
func InitCmd(mbm module.BasicManager, defaultNodeHome string) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if chainID == "" {
				chainID = fmt.Sprintf("keym_2313-%v", tmrand.Str(6))
			}

			if strings.Index(chainID, "keym_2313") != -1 {
			}
			if strings.Index(chainID, "keym_2312") != -1 {
			}
			...
			appState, err := json.MarshalIndent(
				app.GenesisState(mbm.DefaultGenesis(cdc)).
					ConfigureBondDenom(cdc, keytomcfg.BaseDenom), "", " ",
				// ConfigureICA(cdc), "", " ",
			)
			if err != nil {
