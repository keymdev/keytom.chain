...
LEDGER_ENABLED ?= true
BINDIR ?= $(GOPATH)/bin
KEYTOM_BINARY = keytom
KEYTOM_DIR = keytom
BUILDDIR ?= $(CURDIR)/build
SIMAPP = ./app
HTTPS_GIT := https://github.com/keytom/keytom.chain.git
...
ldflags = -X github.com/cosmos/cosmos-sdk/version.Name=keytom \
          -X github.com/cosmos/cosmos-sdk/version.AppName=$(KEYTOM_BINARY) \
...
build-docker:
	...
	$(DOCKER) rm keytom || true
	# create a new container from the latest image
	$(DOCKER) create --name keytom -t -i ${DOCKER_IMAGE}:latest keytom
	# move the binaries to the ./build directory
	# mkdir -p ./build/
	# $(DOCKER) cp keytom:/usr/bin/keytomd ./build/
...
godocs:
	@echo "--> Wait a few seconds and visit http://localhost:6060/pkg/github.com/keytom/keytom.chain/types"
	godoc -http=:6060
...
test-sim-custom-genesis-fast:
	...
	@echo "By default, ${HOME}/.$(KEYTOM_DIR)/config/genesis.json will be used."
	@go test -mod=readonly $(SIMAPP) -run TestFullAppSimulation -Genesis=${HOME}/.$(KEYTOM_DIR)/config/genesis.json \
...
test-sim-custom-genesis-multi-seed: runsim
	...
	@echo "By default, ${HOME}/.$(KEYTOM_DIR)/config/genesis.json will be used."
	@$(BINDIR)/runsim -Genesis=${HOME}/.$(KEYTOM_DIR)/config/genesis.json -SimAppPkg=$(SIMAPP) -ExitOnFail 400 5 TestFullAppSimulation
...
format:
	...
	find . -name '*.go' -type f -not -path "./vendor*" -not -path "*.git*" -not -path "./client/docs/statik/statik.go" -not -name '*.pb.go' | xargs goimports -w -local github.com/keytom/keytom.chain
.PHONY: format
...
PACKAGE_NAME:=github.com/keytom/keytom.chain
