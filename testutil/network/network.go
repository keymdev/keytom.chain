...
func DefaultConfig() Config {
	...
	return Config{
		...
		ChainID:           fmt.Sprintf("keym_%d-1", tmrand.Int63n(9999999999999)+1),
		NumValidators:     4,
		...
		KeyringOptions:    []keyring.Option{keytomhd.Option()},
		PrintMnemonic:     false,
	}
}

// NewAppConstructor returns a new keytom AppConstructor
func NewAppConstructor(encodingCfg params.EncodingConfig) AppConstructor {
	return func(val Validator) servertypes.Application {
		return app.NewKeytom(
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "keytomd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "keytomd")
